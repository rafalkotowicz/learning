"""Test helpers and mocks for the paasio exercise."""
...
class MockException(Exception):
    """Custom exception used by tests to validate propagation and suppression."""


class MockFile(io.BytesIO):
    """In-memory file mock with optional chunking and exception injection."""
...
class MockSock:
    """Socket-like mock with controllable recv/send behavior for tests."""
    ...
    def recv(self, bufsize, flags=0):
        """Receive bytes and emulate socket validation errors."""
        if self.__closed:
    ...
    def send(self, data, flags=0):
        """Send bytes and emulate socket validation errors."""
        if self.__closed:
            raise OSError(errno.EBADF, os.strerror(errno.EBADF))
        if not isinstance(flags, int):
            raise TypeError(
